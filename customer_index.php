...
elseif($page == 'change_password')
{
	   ...
	   && $_POST['send'] == 'send')
	{
		...
		{
			$db->query("UPDATE `" . TABLE_PANEL_CUSTOMERS . "` SET `password`='" . substr(sha1(md5($new_password).$new_password.sha1($new_password)),-30,15) . "' WHERE `customerid`='" . (int)$userinfo['customerid'] . "' AND `password`='" . substr(sha1(md5($old_password).$old_password.sha1($old_password)),-30,15) . "'");
