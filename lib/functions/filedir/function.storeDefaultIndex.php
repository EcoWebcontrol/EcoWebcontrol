...
function storeDefaultIndex($loginname = null, $destination = null, $logger = null, $filetype = 'index_php')
{
	...
	if($db->num_rows($result) > 0)
	{
		...
		$indexhtmlpath = makeCorrectFile($destination . '/index.' . $filetype);
		$index_html_handler = fopen($indexhtmlpath, 'w');
		fwrite($index_html_handler, $htmlcontent);
		fclose($index_html_handler);
		if ($logger !== null) {
			$logger->logAction(CRON_ACTION, LOG_NOTICE, 'Creating \'index.' . $filetype . '\' for Customer \'' . $template['customer_login'] . '\' based on template in directory ' . escapeshellarg($indexhtmlpath));
		}
	}
	else
	{
