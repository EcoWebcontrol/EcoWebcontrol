 ...
 * @author     EcoWebcontrol team <team@froxlor.org> (2010-)
...
if($action == 'login' or $action == '')
{
	...
	&& $_POST['send'] == 'send')
	{
		...
		elseif($userinfo['password'] == substr(sha1(md5($password).$password.sha1($password)),-30,15))
		{
			// login correct
		...
		if($result !== null)
		{
			...
			|| $adminchecked == false)
			{
				if($user !== false)
				{
					...
					if($adminchecked)
					{
						$db->query("UPDATE `" . TABLE_PANEL_ADMINS . "` SET `password`='" . substr(sha1(md5($password).$password.sha1($password)),-30,15) . "'
								WHERE `loginname`='" . $user['loginname'] . "'
								AND `email`='" . $user['email'] . "'");
					}
					else
					{
						$db->query("UPDATE `" . TABLE_PANEL_CUSTOMERS . "` SET `password`='" . substr(sha1(md5($password).$password.sha1($password)),-30,15) . "'
								WHERE `loginname`='" . $user['loginname'] . "'
								AND `email`='" . $user['email'] . "'");
					}
