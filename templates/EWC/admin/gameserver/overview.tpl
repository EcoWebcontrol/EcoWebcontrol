	...
	<article>
		...
		<section>
			...
			<thead>
				<tr>
					<th class="multi">
						<span>{$lng['gameserver']['server_id']}</span>
					</th>
					<th class="multi">
						<span>{$lng['logger']['user']}</span>
					</th>
					<th class="multi">
						<span>{$lng['gameserver']['game']}</span>
					</th>
			...
			<div class="overviewadd">
				<a class="btn btn-inverse" href="{$linker->getLink(array('section' => 'gameserver', 'page' => $page, 'action' => 'add'))}" rel="confirm"><i class="icon-plus icon-white"></i> {$lng['gameserver']['new_gameserver']}</a>
			</div>
			</if>
