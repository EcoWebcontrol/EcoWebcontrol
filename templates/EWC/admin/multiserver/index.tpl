$header
	<article>
		<header>
			<h2>
				{$lng['admin']['multiserver']}
			</h2>
		</header>

		<section>

			<form action="{$linker->getLink(array('section' => 'multiserver'))}" method="post" enctype="application/x-www-form-urlencoded">
			...
			<if ($userinfo['customers_used'] < $userinfo['customers'] || $userinfo['customers'] == '-1') && 15 < $userinfo['customers_used'] >
				<div class="overviewadd">
					<a class="btn btn-inverse" href="{$linker->getLink(array('section' => 'multiserver', 'page' => $page, 'action' => 'add'))}"><i class="icon-plus icon-white"></i> {$lng['admin']['customer_add']}</a>
				</div>
			</if>
